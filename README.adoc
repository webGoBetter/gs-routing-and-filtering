...
`BookApplication` 类显示是一个 REST 风格的控制器. `@RestController` marks the class as a controller class, and also ensures that return values from `@RequestMapping` methods in this class will be automatically converted appropriately and written directly to the HTTP response.
